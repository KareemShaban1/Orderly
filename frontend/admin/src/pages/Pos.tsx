...
function Pos() {
	...
	const { data: menuData, isLoading: menuLoading, error: menuError } = useQuery({
		...
		queryFn: async () => {
			...
			console.log('POS Menu Response:', response.data); // Debug log
			return response.data.data;
		},
			...
			<div className="p-6 max-w-4xl mx-auto">
				<div className="card p-8">
					<div className="text-center mb-6">
						<h2 className="text-2xl font-bold text-slate-900 mb-2">Order Receipt</h2>
						<p className="text-slate-600">Order #{lastOrder.order_number}</p>
					</div>
								...
								return (
									<div key={index} className="flex justify-between items-start">
										<div className="flex-1">
											...
											{item.size && (
												<p className="text-sm text-slate-600 mt-1">Size: <span className="font-medium">{item.size}</span></p>
											)}
										...
										<div className="text-right ml-4">
											<p className="font-bold text-lg text-slate-900">{itemTotalPrice.toFixed(2)} EGP</p>
											{item.quantity > 1 && (
					...
					<div className="space-y-2 mb-4">
						...
						<div className="flex justify-between text-2xl font-bold border-t-2 border-slate-300 pt-3 mt-2">
							<span>Total:</span>
	...
	return (
		<div className="h-full flex flex-col bg-slate-50">
			...
			<div className="bg-white border-b border-slate-200 p-4">
				...
				{success && (
					<div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 flex items-center justify-between">
				...
				<div className="flex items-center space-x-4">
					<div className="flex space-x-2">
						<button
							...
							className={`px-4 py-2 rounded-lg font-medium transition-colors ${customerType === 'walk_in'
						...
						<button
							...
							className={`px-4 py-2 rounded-lg font-medium transition-colors ${customerType === 'table'
								? 'bg-slate-900 text-white'
								: 'bg-slate-100 text-slate-700 hover:bg-slate-200'
								}`}
						>
			...
			<div className="flex-1 flex overflow-hidden">
				...
				<div className="flex-1 flex flex-col overflow-hidden">
					...
					<div className="flex-1 overflow-y-auto p-4">
						...
						{menuError && (
							<div className="flex items-center justify-center h-full">
								<p className="text-red-600">Error loading menu: {String(menuError)}</p>
							</div>
						)}
						{!menuLoading && !menuError && (!menuData || filteredMenu.length === 0) && (
							<div className="flex items-center justify-center h-full">
								<div className="text-center">
									<p className="text-slate-600 mb-2">No menu items available.</p>
									<p className="text-sm text-slate-500">Please add categories and items in Menu Management.</p>
								</div>
							</div>
						)}
							...
							<div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
								{(selectedCategory
									...
									return (
										<div
											key={item.id}
											className="bg-white rounded-xl border-2 border-slate-200 hover:border-slate-900 hover:shadow-lg transition-all flex flex-col cursor-pointer group"
											onClick={(e) => {
												e.stopPropagation();
												addToCart(item);
											}}
										>
											...
											<div className="relative h-48 bg-slate-100 rounded-t-xl overflow-hidden">
												...
												{/* Quantity Badge */}
												{state.quantity > 1 && (
													<div className="absolute top-2 right-2 bg-slate-900 text-white rounded-full w-8 h-8 flex items-center justify-center font-bold text-sm shadow-lg">
														{state.quantity}
													</div>
												)}
											</div>
											...
											<div className="p-3 flex-1 flex flex-col">
												<h3 className="font-bold text-slate-900 text-base mb-1 line-clamp-1">{item.name}</h3>
														...
														<div className="flex flex-wrap gap-2">
															{item.sizes!.map((size: any) => (
																<button
																	...
																	className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-all ${state.size === size.name
												...
												{hasAddons && (
													<div className="mb-3">
														<div className="text-xs font-medium text-slate-600 mb-2">Add-ons:</div>
														<div className="space-y-1.5 max-h-32 overflow-y-auto">
															{item.addons!.map((addon: any) => (
																<label
																	key={addon.id}
																	className="flex items-center justify-between p-2 rounded-lg border border-slate-200 hover:border-slate-300 cursor-pointer transition-colors"
																...
																>
																	<div className="flex items-center space-x-2">
																		...
																		/>
																		<span className="text-sm text-slate-900">{addon.name}</span>
																	</div>
																	<span className="text-sm font-semibold text-slate-700">+{addon.price.toFixed(2)}</span>
																</label>
															))}
														</div>
													</div>
													...
													<div className="flex items-center flex-col justify-between gap-3">
														<div className="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2">
															<button
																...
																className="w-8 h-8 rounded-lg bg-white border-2 border-slate-300 flex items-center justify-center hover:bg-slate-100 font-bold text-slate-700 text-lg transition-colors"
															>
																−
															</button>
															<span className="w-10 text-center font-bold text-slate-900 text-lg">{state.quantity}</span>
															<button
																onClick={(e) => {
																	e.stopPropagation();
																	updateItemState(item.id, { quantity: state.quantity + 1 });
														...
														<button
															...
															className="flex-1 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-4 rounded-lg text-base transition-colors shadow-md"
														>
															Add to Cart
														</button>
													</div>
				...
				<div className="w-96 bg-white border-l border-slate-200 flex flex-col">
								...
								<div key={index} className="bg-slate-50 rounded-lg p-3">
									...
									<div className="flex items-center space-x-2">
										<button
											...
											className="w-8 h-8 rounded border border-slate-300 flex items-center justify-center hover:bg-slate-200"
										>
											-
										</button>
					...
					<div className="p-4 border-t border-slate-200 space-y-2">
						...
						<div className="flex justify-between text-sm">
							...
							<span>{totals.serviceCharge.toFixed(2)} EGP</span>
						</div>
						...
						<div className="space-y-2 mt-4">
							<button
								...
								className="w-full btn btn-primary py-4 text-lg font-bold flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
							>
							...
							<div className="grid grid-cols-2 gap-2">
								<button
									...
									disabled={cart.length === 0 || createOrderMutation.isPending || !selectedBranch}
									className="btn btn-primary py-3 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
								>
									<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							>
								...
								<span>Hold Order (Pay Later)</span>
							</button>
						</div>
					</div>
				</div>
